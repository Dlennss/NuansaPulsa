...
export function EwalletProviderPicker({ items }: EwalletProviderPickerProps) {
  ...
  return (
    <div className="space-y-3 pb-28">
      <section className="overflow-hidden rounded-[28px] border border-rose-950/5 bg-linear-to-br from-white via-red-50/80 to-amber-50/70 p-3 shadow-[0_18px_42px_rgba(151,14,32,0.12)]">
        <div className="flex items-center gap-3">
          <span className="grid h-13 w-13 shrink-0 place-items-center rounded-[20px] bg-linear-to-br from-[#d70717] to-[#ff6a00] text-amber-200 shadow-[0_14px_28px_rgba(151,14,32,0.18)]">
          ...
          <button
            type="button"
            onClick={() => setDest("")}
            className="inline-flex h-9 items-center gap-1.5 rounded-full bg-rose-50 px-3 text-[11px] font-black text-rose-700 ring-1 ring-rose-200"
          >
        ...
        <label className="mt-4 flex h-14 items-center overflow-hidden rounded-2xl border border-rose-500/25 bg-white shadow-inner focus-within:border-rose-500 focus-within:ring-4 focus-within:ring-rose-100">
          <span className="grid h-full w-14 shrink-0 place-items-center border-r border-rose-100 text-sm font-black text-rose-700">
            +62
          </span>
          <input
            value={dest}
      ...
      <section className="overflow-hidden rounded-[30px] border border-rose-950/5 bg-linear-to-br from-white via-[#f8fffb] to-rose-50/85 p-3 shadow-[0_18px_42px_rgba(151,14,32,0.12)]">
        <div className="flex items-center justify-between gap-3 px-1 pb-3 pt-1">
          ...
          {selectedItem ? (
            <span className="max-w-24 truncate rounded-full bg-[#d70717] px-3 py-1.5 text-[10px] font-black text-amber-200 shadow-[0_10px_18px_rgba(151,14,32,0.16)]">
              {selectedItem.title}
            </span>
          ) : null}
        </div>
              ...
              return (
                <button
                  ...
                  className={`group relative min-h-[104px] overflow-hidden rounded-[23px] border px-2.5 pb-2 pt-2.5 text-left transition duration-300 hover:-translate-y-0.5 ${
                    selected
                      ? "border-amber-300 bg-[#d70717] shadow-[0_18px_34px_rgba(151,14,32,0.24)] ring-2 ring-amber-200/70"
                      : "border-white bg-white shadow-[0_14px_30px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/70 hover:border-rose-200 hover:shadow-[0_18px_34px_rgba(151,14,32,0.13)]"
                  }`}
                >
                  ...
                  {selected ? (
                    <span className="absolute right-2 top-2 z-10 grid h-5.5 w-5.5 place-items-center rounded-full bg-amber-300 text-[#d70717] shadow-[0_8px_16px_rgba(0,0,0,0.18)]">
                      <CheckCircle2 className="h-3.5 w-3.5" strokeWidth={2.6} />
                    </span>
                  ) : null}
                  <div className={`relative mx-auto grid h-12.5 w-12.5 place-items-center overflow-hidden rounded-[18px] bg-white p-2.5 shadow-[0_12px_24px_rgba(15,23,42,0.13)] ring-1 ${selected ? "ring-amber-200" : "ring-slate-100"}`}>
                    {item.imageSrc ? (
        ...
        <Link
          ...
          className={`group relative mt-4 flex h-13 items-center justify-center gap-2 overflow-hidden rounded-[22px] text-sm font-black transition ${
            selectedItem
              ? "bg-linear-to-r from-[#d70717] via-[#b50718] to-[#d70717] text-white shadow-[0_18px_34px_rgba(151,14,32,0.28)] ring-1 ring-white/20"
              : "pointer-events-none bg-slate-200 text-slate-400"
          }`}
        >
          <span className="absolute inset-y-0 left-0 w-1/2 bg-linear-to-r from-rose-400/28 to-transparent" />
          <span className="absolute -right-8 -top-10 h-24 w-24 rounded-full bg-amber-300/35 blur-xl transition group-hover:bg-amber-200/45" />
          <span className="relative tracking-wide drop-shadow-sm">Lihat Produk & Nominal</span>
          <span className="relative grid h-6 w-6 place-items-center rounded-full bg-amber-300 text-[#d70717] shadow-[0_8px_16px_rgba(0,0,0,0.18)] transition group-hover:translate-x-0.5">
