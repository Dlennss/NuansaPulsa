...
function getPaymentHelper(methodId: string, applicationId: number) {
  if (methodId === "qris") {
    return {
      ...
      desc: "Scan QRIS dari kanal NuansaPulsa, lalu upload bukti pembayaran.",
      rows: [
  ...
  if (methodId === "transfer") {
    return {
      ...
      rows: [
        ["Bank", "BCA"],
        ["Atas Nama", "NuansaPulsa"],
      ],
    };
  }
  return {
    title: "Penagihan Offline",
    value: "Marketing NuansaPulsa",
...
function getApplicantLabel(item: AgentCreditApplication) {
  ...
  return {
    name: String(data.agent_name || data.nama_lengkap || "Agent NuansaPulsa"),
...
export function UserCreditBillPayLaterContent({ bills }: Props) {
  ...
  async function copyPaymentValue() {
    if (!paymentHelper.value || paymentHelper.value === "Saldo akun NuansaPulsa") return;
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-4">
      <section className="relative overflow-hidden rounded-[30px] bg-[linear-gradient(135deg,#b20717,#d70717_58%,#ff6a00_135%)] p-5 text-white shadow-[0_22px_50px_rgba(215,7,23,0.22)]">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-100">Tagihan Kredit Agent</p>
          ...
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-[#d70717]">
            <ReceiptText className="h-6 w-6" />
          </span>
        </div>
      </section>

      <section className="rounded-[28px] border border-red-100 bg-white p-5 shadow-[0_18px_42px_rgba(151,14,32,0.08)]">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-red-600">Ringkasan Tagihan</p>
        <div className="mt-3 flex items-end justify-between gap-3">
          ...
          <span className={totalOutstanding <= 0 ? "rounded-full bg-red-100 px-3 py-1.5 text-[10px] font-black text-red-700" : "rounded-full bg-amber-100 px-3 py-1.5 text-[10px] font-black text-amber-700"}>
            {totalOutstanding <= 0 ? "Bersih" : "Belum lunas"}
          </span>
        </div>
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="rounded-2xl bg-red-50 px-3 py-3">
            <p className="text-[9px] font-black uppercase text-red-700/70">Aktif</p>
            <p className="mt-1 text-sm font-black text-slate-950">{activeBills.length}</p>
          </div>
          <div className="rounded-2xl bg-[#fff6f4] px-3 py-3">
            <p className="text-[9px] font-black uppercase text-[#d70717]/70">Lunas</p>
      ...
      {bills.length ? (
        <>
          <section className="rounded-[28px] border border-red-100 bg-white p-4 shadow-[0_18px_42px_rgba(151,14,32,0.08)]">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-base font-black text-slate-950">Daftar Tagihan</h2>
              <span className="rounded-full bg-amber-100 px-3 py-1 text-[10px] font-black text-[#d70717]">{displayedBills.length} data</span>
            </div>
            ...
            <div className="space-y-2">
              {displayedBills.length ? displayedBills.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={selected ? "flex w-full items-center gap-3 rounded-[24px] border border-red-300 bg-[linear-gradient(135deg,#ecfdf5,#ffffff)] p-3 shadow-[0_12px_24px_rgba(215,7,23,0.08)]" : "flex w-full items-center gap-3 rounded-[24px] border border-slate-200 bg-white p-3"}
                  >
                    <span className={selected ? "grid h-12 w-12 shrink-0 place-items-center rounded-[19px] bg-[#d70717] text-white shadow-[0_10px_20px_rgba(215,7,23,0.20)]" : "grid h-12 w-12 shrink-0 place-items-center rounded-[19px] bg-slate-100 text-slate-500"}>
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-1 block text-sm font-black text-[#d70717]">{formatIDR(outstanding)}</span>
                      <span className="mt-0.5 block text-[10px] font-semibold text-slate-400">
                        Kredit sudah dicairkan ke saldo utama
                      </span>
                    </span>
                    <span className="flex shrink-0 flex-col items-end gap-2">
                      <span className={itemPaid ? "rounded-full bg-red-100 px-2.5 py-1 text-[9px] font-black text-red-700" : outstanding > 0 ? "rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-black text-amber-700" : "rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-black text-red-700"}>
                        {itemPaid ? "Lunas" : outstanding > 0 ? "Belum Lunas" : "Aktif"}
                      </span>
                      {outstanding > 0 ? (
                        <button type="button" onClick={() => selectBill(item.id, true)} className="rounded-full bg-[#d70717] px-3 py-2 text-[10px] font-black text-white">
                          Bayar
                        </button>
                      ) : null}
                      <Link href="/user/saldo/kredit-agent" className="rounded-full border border-red-100 bg-red-50 px-3 py-2 text-[10px] font-black text-[#d70717]">
                        Detail
                      </Link>
                    </span>
                  </div>
                ...
                <div className="rounded-[22px] border border-dashed border-red-200 bg-red-50/40 px-4 py-8 text-center">
          ...
          {paymentOpen && selectedBill ? (
            <section className="overflow-hidden rounded-[28px] border border-red-100 bg-white shadow-[0_18px_42px_rgba(151,14,32,0.08)]">
              <div className="bg-[linear-gradient(135deg,#f8fffb,#eefcf4)] p-4">
                <div className="flex items-center gap-3">
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-[#d70717] ring-1 ring-red-100">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <div className="rounded-2xl bg-white px-3 py-2 ring-1 ring-red-100">
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-[linear-gradient(90deg,#d70717,#ff6a00)]" style={{ width: `${usageProgress}%` }} />
                  </div>
                </div>
              </div>
            </section>
              ...
              {paymentMethods.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={selected ? "flex w-full items-center gap-3 rounded-[22px] border border-red-300 bg-red-50 p-3 text-left" : "flex w-full items-center gap-3 rounded-[22px] border border-slate-200 bg-white p-3 text-left transition hover:bg-slate-50"}
                  >
                    <span className={selected ? "grid h-11 w-11 place-items-center rounded-2xl bg-[#d70717] text-white" : "grid h-11 w-11 place-items-center rounded-2xl bg-slate-100 text-slate-500"}>
                    ...
                    {selected ? <CheckCircle2 className="h-5 w-5 text-[#d70717]" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}
                  </button>
                );
              })}
            </div>
          ...
          <section className="rounded-[28px] border border-red-100 bg-white p-4 shadow-[0_18px_42px_rgba(151,14,32,0.08)]">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-100 text-[#d70717]">
              ...
              <MethodIcon className="h-5 w-5 text-[#d70717]" />
            </div>
            <div className="mt-4 space-y-2 rounded-[22px] bg-slate-50 p-3 text-sm">
              <div className="mb-3 rounded-[20px] border border-red-100 bg-white p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[9px] font-black uppercase tracking-[0.14em] text-red-600">{paymentHelper.title}</p>
                  ...
                  {selectedMethod !== "offline" ? (
                    <button
                      ...
                      className="h-9 shrink-0 rounded-full bg-red-50 px-3 text-[10px] font-black text-[#d70717] ring-1 ring-red-100"
                    >
                      Salin
                    </button>
                  ) : null}
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {paymentHelper.rows.map(([label, value]) => (
                    <div key={label} className="rounded-2xl bg-red-50/70 px-3 py-2">
                      <p className="text-[9px] font-black uppercase text-red-700/70">{label}</p>
              ...
              <div className="flex justify-between gap-3">
                ...
                <span className="font-black text-[#d70717]">Gratis</span>
              </div>
            ...
            <label className="mt-4 flex cursor-pointer items-center gap-3 rounded-[22px] border border-dashed border-red-300 bg-red-50/60 p-3 transition hover:bg-red-50">
              ...
              <span className={proofFile ? "rounded-full bg-red-600 px-3 py-1 text-[10px] font-black text-white" : "rounded-full bg-white px-3 py-1 text-[10px] font-black text-[#d70717] ring-1 ring-red-100"}>
            ...
            <button
              ...
              className="mt-4 flex h-13 w-full items-center justify-center gap-2 rounded-[20px] bg-[linear-gradient(135deg,#b20717,#d70717,#ff6a00)] text-sm font-black text-white shadow-[0_18px_34px_rgba(215,7,23,0.22)] disabled:opacity-50"
            >
        ...
        <section className="grid min-h-[300px] place-items-center rounded-[28px] border border-dashed border-red-200 bg-white px-6 text-center shadow-[0_18px_42px_rgba(151,14,32,0.06)]">
          <div>
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-red-50 text-[#d70717]">
