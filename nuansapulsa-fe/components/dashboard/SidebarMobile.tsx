...
export function SidebarMobile({ sections, open, onClose, onLogout, contextLabel = "Control Center" }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-sm md:hidden" onClick={onClose}>
      <aside
        className="flex h-full w-72 flex-col border-r border-red-950/20 bg-[radial-gradient(circle_at_20%_0%,rgba(255,204,0,0.22),transparent_30%),linear-gradient(180deg,#8f0614_0%,#d70717_50%,#ff6a00_120%)] p-4 shadow-[0_24px_60px_rgba(151,14,32,0.20)]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <BrandLogo variant="dark" />
            <p className="mt-2 text-center text-[9px] font-black uppercase tracking-[0.16em] text-yellow-100">{contextLabel}</p>
          </div>
          <button
            type="button"
            className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-white/25 bg-white text-[#d70717]"
            onClick={onClose}
