...
export default function OperatorManualAgentEntryPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fff6f4] p-3 text-slate-950 sm:p-6 lg:p-8">
      <section className="mx-auto w-full max-w-6xl overflow-hidden rounded-[28px] border border-red-950/[0.06] bg-white shadow-[0_22px_56px_rgba(151,14,32,0.1)]">
        <header className="overflow-hidden bg-[radial-gradient(circle_at_88%_8%,rgba(255,196,0,0.34),transparent_30%),linear-gradient(135deg,#b20717_0%,#e50917_54%,#ff6a00_118%)] p-4 text-white sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-yellow-100">Operator Kredit</p>
            ...
            <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white text-[#d70717] shadow-[0_12px_24px_rgba(0,0,0,0.12)]">
              <Upload className="h-6 w-6" />
            </span>
          </div>
        </header>
        <div className="p-4 sm:p-6">
          {notice ? <div className="mb-4 flex items-start justify-between gap-3 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-xs font-bold text-[#d70717]"><span>{notice}</span><button type="button" onClick={() => setNotice("")} aria-label="Tutup"><X className="h-4 w-4" /></button></div> : null}
          {selectedMarketing ? <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-red-50 text-[#d70717]"><UsersRound className="h-5 w-5" /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#d70717]">Agent Binaan</p><h2 className="truncate text-lg font-black text-slate-950">{selectedMarketing.name}</h2><p className="truncate text-xs font-semibold text-slate-500">{selectedMarketing.email}</p></div></div><button type="button" onClick={() => setSelectedMarketingKey("")} className="inline-flex h-10 items-center justify-center gap-2 rounded-2xl border border-red-100 bg-white px-4 text-xs font-black text-[#d70717] shadow-sm hover:bg-red-50"><ArrowLeft className="h-4 w-4" /> Kembali ke Marketing</button></div> : <label className="flex h-12 max-w-lg items-center gap-2 rounded-2xl border border-red-950/[0.06] bg-[#fffafa] px-4 shadow-sm focus-within:border-[#d70717] focus-within:bg-white focus-within:ring-4 focus-within:ring-red-100"><Search className="h-4 w-4 text-[#d70717]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari nama atau email marketing" className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none placeholder:text-slate-400" /></label>}
        </div>
        <div className="overflow-x-auto border-t border-red-950/[0.06]">
          {!selectedMarketing ? <table className="w-full min-w-[680px] text-left text-xs">
            <thead className="bg-[#fff1ed] text-[10px] font-black uppercase tracking-[0.12em] text-red-900"><tr><th className="px-5 py-3">Akun Marketing</th><th className="px-4 py-3">Jumlah Agent</th><th className="px-4 py-3">Sudah Migrasi</th><th className="px-4 py-3">Belum Migrasi</th><th className="px-5 py-3 text-right">Aksi</th></tr></thead>
            <tbody>
              {visibleMarketingGroups.map((group) => { const migrated = group.agents.filter((agent) => migrationByMember.has(agent.id)).length; return <tr key={group.key} className="border-t border-slate-100 hover:bg-red-50/40"><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-red-50 font-black text-[#d70717]">{group.id > 0 ? group.name.slice(0, 1).toUpperCase() : "-"}</span><div><p className="font-black text-slate-950">{group.name}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{group.email}{group.id > 0 ? ` · ID #${group.id}` : ""}</p></div></div></td><td className="px-4 py-4 font-black text-slate-700">{group.agents.length} agent</td><td className="px-4 py-4"><span className="rounded-full bg-red-100 px-2.5 py-1 text-[10px] font-black text-[#d70717]">{migrated} agent</span></td><td className="px-4 py-4"><span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-black text-amber-800">{group.agents.length - migrated} agent</span></td><td className="px-5 py-4 text-right"><button type="button" onClick={() => setSelectedMarketingKey(group.key)} className="inline-flex h-9 items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#d70717,#ff6a00)] px-4 text-xs font-black text-white shadow-[0_10px_22px_rgba(215,7,23,0.2)] hover:brightness-95"><UsersRound className="h-4 w-4" /> Lihat Agent</button></td></tr>; })}
          ...
          </table> : <table className="w-full min-w-[820px] text-left text-xs"><thead className="bg-[#fff1ed] text-[10px] font-black uppercase tracking-[0.12em] text-red-900"><tr><th className="px-5 py-3">Akun Agent</th><th className="px-4 py-3">Nama Toko</th><th className="px-4 py-3">Nomor Telepon</th><th className="px-4 py-3">Status Migrasi</th><th className="px-5 py-3 text-right">Aksi</th></tr></thead><tbody>{selectedMarketing.agents.map((agent) => { const migration = migrationByMember.get(agent.id); return <tr key={agent.id} className="border-t border-slate-100 hover:bg-red-50/40"><td className="px-5 py-4"><p className="font-black text-slate-950">{agent.name || "Agent"}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{agent.email} · ID #{agent.id}</p></td><td className="px-4 py-4 font-semibold text-slate-600">{recordText(migration?.applicant_data, "store_name") || agent.store_name || "Belum diisi"}</td><td className="px-4 py-4 font-semibold text-slate-600">{recordText(migration?.applicant_data, "whatsapp") || migration?.member_phone || agent.phone || "-"}</td><td className="px-4 py-4">{migration ? <div><span className="inline-flex rounded-full bg-red-100 px-2.5 py-1 text-[10px] font-black text-[#d70717]">Sudah dimigrasikan</span><p className="mt-1 text-[10px] font-semibold text-slate-500">{statusLabel(migration.status)} · KRD-{String(migration.id).padStart(8, "0")} · {formatDate(migration.created_at)}</p></div> : <span className="text-xs font-bold text-slate-400">Belum dimigrasikan</span>}</td><td className="px-5 py-4 text-right"><button type="button" onClick={() => openMigration(agent, migration)} className="inline-flex h-9 items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#d70717,#ff6a00)] px-4 text-xs font-black text-white shadow-[0_10px_22px_rgba(215,7,23,0.2)] hover:brightness-95"><Upload className="h-4 w-4" />{migration ? "Edit Migrasi" : "Migrasi Data"}</button></td></tr>; })}</tbody></table>}
        </div>
        <footer className="border-t border-red-950/[0.06] px-5 py-3 text-[11px] font-bold text-slate-500">{selectedMarketing ? `${selectedMarketing.agents.length} agent binaan` : `${visibleMarketingGroups.length} akun marketing`}</footer>
      </section>
      ...
      {selectedAgent ? <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-2 backdrop-blur-sm sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setSelectedAgent(null); }}>
        <section role="dialog" aria-modal="true" aria-labelledby="migration-title" className="flex max-h-[calc(100dvh-16px)] w-full max-w-3xl flex-col overflow-hidden rounded-[28px] border border-white/60 bg-white shadow-[0_30px_90px_rgba(151,14,32,0.28)] sm:max-h-[calc(100dvh-40px)]">
          <header className="flex shrink-0 items-center justify-between gap-4 border-b border-red-950/[0.06] bg-[#fff7f5] px-4 py-4 sm:px-6"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#d70717]">{migrationByMember.has(selectedAgent.id) ? "Edit Migrasi" : "Migrasi Dokumen"}</p><h2 id="migration-title" className="mt-1 text-xl font-black text-slate-950">{selectedAgent.name}</h2></div><button type="button" disabled={saving} onClick={() => setSelectedAgent(null)} className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-slate-500 shadow-sm disabled:opacity-50" aria-label="Tutup"><X className="h-5 w-5" /></button></header>
          <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">{documentFields.map((field) => { const image = documents[field.key]; return <label key={field.key} className={`group relative overflow-hidden rounded-3xl border transition ${image ? "border-red-300 bg-red-50" : "border-dashed border-red-200 bg-white hover:bg-red-50/40"}`}><input type="file" accept="image/*" className="sr-only" disabled={Boolean(preparing) || saving} onChange={(event) => { void selectImage(field.key, event.target.files?.[0]); event.currentTarget.value = ""; }} />{image ? <div className="relative aspect-[16/9] bg-slate-100"><img src={image.data_url} alt={field.label} className="h-full w-full object-contain" /><span className="absolute right-2 top-2 rounded-full bg-[linear-gradient(135deg,#d70717,#ff6a00)] px-2 py-1 text-[9px] font-black text-white">Tersimpan</span></div> : <div className="grid aspect-[16/9] place-items-center bg-[#fff7f5]"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-[#d70717] shadow-sm">{preparing === field.key ? <Loader2 className="h-5 w-5 animate-spin" /> : <Camera className="h-5 w-5" />}</span></div>}<div className="flex items-center gap-3 p-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-red-50 text-[#d70717]"><FileImage className="h-4 w-4" /></span><div><p className="text-xs font-black text-slate-950">{field.label}</p><p className="mt-0.5 text-[10px] font-semibold text-slate-500">{image ? "Ketuk untuk mengganti foto" : field.description}</p></div></div></label>; })}</div>
          ...
          <footer className="shrink-0 border-t border-red-950/[0.06] bg-white p-4 sm:px-6"><button type="button" disabled={saving || Boolean(preparing)} onClick={() => void submit()} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#d70717,#ff6a00)] px-5 text-sm font-black text-white shadow-[0_12px_26px_rgba(215,7,23,0.22)] transition hover:brightness-95 disabled:opacity-50">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <CheckCircle2 className="h-5 w-5" />}{saving ? "Mengaktifkan Kredit..." : migrationByMember.has(selectedAgent.id) ? "Simpan Perubahan" : "Simpan & Aktifkan Kredit"}</button></footer>
        </section>
      </div> : null}
      ...
      {success ? <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setSuccess(null); }}>
        <section role="alertdialog" aria-modal="true" aria-labelledby="migration-success-title" className="w-full max-w-sm overflow-hidden rounded-[28px] border border-red-950/[0.06] bg-white shadow-[0_30px_90px_rgba(151,14,32,0.28)]">
          <div className="bg-[linear-gradient(135deg,#b20717,#e50917,#ff6a00)] px-6 py-7 text-center text-white">
            <span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-white/95 text-[#d70717] shadow-lg"><CheckCircle2 className="h-9 w-9" /></span>
            <p className="mt-4 text-[10px] font-black uppercase tracking-[0.18em] text-yellow-100">Migrasi Data Agent</p>
          ...
          <div className="p-5 text-center sm:p-6">
            ...
            <div className="mt-4 rounded-xl border border-red-950/[0.06] bg-red-50 px-4 py-3">
              <p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#d70717]">ID Kredit</p>
              <p className="mt-1 text-base font-black text-slate-950">{success.creditID}</p>
            </div>
            <button type="button" onClick={() => setSuccess(null)} className="mt-5 h-11 w-full rounded-2xl bg-[linear-gradient(135deg,#d70717,#ff6a00)] px-5 text-sm font-black text-white transition hover:brightness-95">Selesai</button>
          </div>
        </section>
      </div> : null}
    </main>
...
function Field({ label, value, onChange, placeholder, inputMode, hint }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; inputMode?: "tel" | "numeric"; hint?: string }) {
  return <label className="block"><span className="text-[10px] font-black uppercase tracking-[0.1em] text-[#d70717]">{label}</span><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} inputMode={inputMode} className="mt-1.5 h-12 w-full rounded-2xl border border-red-950/[0.08] bg-[#fffafa] px-4 text-sm font-bold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-[#d70717] focus:bg-white focus:ring-4 focus:ring-red-100" />{hint ? <span className="mt-1.5 block text-[10px] font-semibold text-slate-500">{hint}</span> : null}</label>;
}
