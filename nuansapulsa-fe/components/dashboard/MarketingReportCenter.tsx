...
function agentName(item: AgentCreditApplication) {
  return applicantValue(item, "agent_name", item.member_name || item.agent_name || "Agent NuansaPulsa");
}
...
export function MarketingReportCenter({ applications }: Props) {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text(`NuansaPulsa - ${reportTitle}`, 32, 34);
      doc.setFontSize(9);
