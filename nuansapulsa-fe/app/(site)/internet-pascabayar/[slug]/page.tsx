...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "Internet Pascabayar | NuansaPulsa",
      description: "Cek dan bayar tagihan internet pascabayar di NuansaPulsa.",
      path: "/internet-pascabayar",
    });
  }

  return buildPageMetadata({
    title: `${toTitleCase(brand.nama)} | Internet Pascabayar NuansaPulsa`,
    description: `Cek dan bayar tagihan ${toTitleCase(brand.nama)} di NuansaPulsa dengan proses yang ringkas.`,
    path: `/internet-pascabayar/${slug}`,
    keywords: [`${String(brand.nama).toLowerCase()} pascabayar`, `bayar ${String(brand.nama).toLowerCase()}`, "internet pascabayar", "nuansapulsa"],
  });
}
...
export default async function InternetPascabayarSlugPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${brandTitle} | Internet Pascabayar NuansaPulsa`,
    description: `Cek dan bayar tagihan ${brandTitle} di NuansaPulsa dengan proses yang ringkas.`,
    path: `/internet-pascabayar/${slug}`,
