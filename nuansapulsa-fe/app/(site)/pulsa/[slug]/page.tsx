...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!pulsaCategory) {
    return buildPageMetadata({
      title: "Pulsa Operator | NuansaPulsa",
      description: "Isi pulsa operator favorit Anda di NuansaPulsa.",
      path: "/pulsa",
    });
  }
  ...
  return buildPageMetadata({
    title: `Isi Pulsa ${brandTitle} Online | NuansaPulsa`,
    description: lowestPrice
      ? `Isi pulsa ${brandTitle} online di NuansaPulsa dengan nominal lengkap. Harga mulai Rp ${lowestPrice.toLocaleString("id-ID")} dan transaksi cepat.`
      : `Isi pulsa ${brandTitle} online di NuansaPulsa dengan nominal lengkap dan transaksi cepat.`,
    path: `/pulsa/${slug}`,
    keywords: [`pulsa ${brand.nama.toLowerCase()}`, `isi pulsa ${brand.nama.toLowerCase()}`, "nuansapulsa"],
...
export default async function GuestPulsaBrandPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `Isi Pulsa ${brandTitle} Online | NuansaPulsa`,
    description: `Isi pulsa ${brandTitle} online di NuansaPulsa dengan nominal lengkap dan transaksi cepat.`,
    path: `/pulsa/${slug}`,
  ...
  const faqJsonLd = buildFaqJsonLd([
    {
      question: `Apakah nominal pulsa ${brandTitle} di NuansaPulsa lengkap?`,
      answer: `Ya. NuansaPulsa menampilkan pilihan nominal pulsa ${brandTitle} yang aktif agar pembeli bisa memilih sesuai kebutuhan.`,
    },
    {
      question: `Bagaimana cara beli pulsa ${brandTitle} di NuansaPulsa?`,
      answer: `Pilih nominal pulsa ${brandTitle}, masukkan nomor tujuan, lalu lanjutkan pembayaran sesuai metode yang tersedia.`,
    },
    {
      question: `Apakah pembelian pulsa ${brandTitle} di NuansaPulsa bisa untuk calon member dan member?`,
      answer: `Bisa. Halaman ini bisa dipakai pembeli umum, dan member NuansaPulsa juga bisa memanfaatkan pilihan produk yang sama untuk transaksi harian.`,
    },
  ]);
