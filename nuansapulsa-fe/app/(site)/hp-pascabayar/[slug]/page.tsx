...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "HP Pascabayar | NuansaPulsa",
      description: "Cek dan bayar tagihan HP pascabayar di NuansaPulsa.",
      path: "/hp-pascabayar",
    });
  }

  return buildPageMetadata({
    title: `${toTitleCase(brand.nama)} | HP Pascabayar NuansaPulsa`,
    ...
    keywords: [`${String(brand.nama).toLowerCase()} pascabayar`, `bayar ${String(brand.nama).toLowerCase()}`, "hp pascabayar", "nuansapulsa"],
  });
}
...
export default async function HPPascabayarSlugPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${brandTitle} | HP Pascabayar NuansaPulsa`,
