...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      ...
      description: "Cek dan bayar tagihan PDAM di NuansaPulsa.",
      path: "/pdam",
    });
  }

  return buildPageMetadata({
    title: `${brand.nama} | Tagihan PDAM NuansaPulsa`,
    description: `Cek dan bayar tagihan ${brand.nama} di NuansaPulsa dengan proses cepat dan ringkas.`,
    path: `/pdam/${slug}`,
    keywords: [`tagihan ${brand.nama.toLowerCase()}`, `bayar ${brand.nama.toLowerCase()}`, "pdam online", "nuansapulsa"],
  });
}
