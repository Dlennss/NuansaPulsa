...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!dataCategory) {
    return buildPageMetadata({
      title: "Paket Data Operator | NuansaPulsa",
      description: "Beli paket data operator favorit Anda di NuansaPulsa.",
      path: "/paket-data",
    });
  }
  ...
  return buildPageMetadata({
    title: `Paket Data ${brandTitle} Murah | NuansaPulsa`,
    description: lowestPrice
      ? `Beli paket data ${brandTitle} di NuansaPulsa dengan pilihan kuota aktif. Harga mulai Rp ${lowestPrice.toLocaleString("id-ID")} dan transaksi cepat.`
      : `Beli paket data ${brandTitle} di NuansaPulsa dengan pilihan kuota dan masa aktif yang jelas.`,
    path: `/paket-data/${slug}`,
    keywords: [`paket data ${brand.nama.toLowerCase()}`, `kuota ${brand.nama.toLowerCase()}`, "nuansapulsa"],
...
export default async function GuestPaketDataBrandPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `Paket Data ${brandTitle} Murah | NuansaPulsa`,
    description: `Beli paket data ${brandTitle} di NuansaPulsa dengan pilihan kuota dan masa aktif yang jelas.`,
    path: `/paket-data/${slug}`,
  ...
  const faqJsonLd = buildFaqJsonLd([
    {
      question: `Apakah paket data ${brandTitle} di NuansaPulsa tersedia dalam banyak pilihan kuota?`,
      answer: `Ya. NuansaPulsa menampilkan paket data ${brandTitle} yang aktif dengan pilihan kuota, masa aktif, dan grup produk yang lebih jelas.`,
    },
    {
      question: `Bagaimana cara beli paket data ${brandTitle} di NuansaPulsa?`,
    ...
    },
    {
      ...
      answer: `Halaman ini bisa dipakai pembeli umum maupun member NuansaPulsa untuk kebutuhan transaksi harian dan penjualan ulang.`,
    },
  ]);
