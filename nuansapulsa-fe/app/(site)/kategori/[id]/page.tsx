...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (universalService) {
    return buildPageMetadata({
      title: `${universalService === "esim-roaming" ? "eSIM & Roaming" : "HP Pascabayar"} | NuansaPulsa`,
      description: "Layanan digital NuansaPulsa.",
      path: `/kategori/${id}`,
    });
  }
  ...
  if (!category) {
    return buildPageMetadata({
      title: "Kategori Produk | NuansaPulsa",
      description: "Lihat kategori produk NuansaPulsa.",
      path: "/kategori",
    });
  }

  return buildPageMetadata({
    title: `${category.nama} | NuansaPulsa`,
    description: `Lihat pilihan brand dan produk ${category.nama} di NuansaPulsa.`,
    path: `/kategori/${id}`,
    keywords: [String(category.nama).toLowerCase(), "produk nuansapulsa", "nuansapulsa"],
  });
}
...
export default async function GuestKategoriPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${categoryName} | NuansaPulsa`,
    description: `Lihat pilihan brand dan produk ${categoryName} di NuansaPulsa.`,
    path: `/kategori/${id}`,
