      ...
      <div className="mx-auto w-full max-w-6xl">
        <header className="overflow-hidden rounded-[28px] bg-[radial-gradient(circle_at_88%_8%,rgba(255,196,0,0.36),transparent_30%),linear-gradient(135deg,#b20717_0%,#e50917_54%,#ff6a00_118%)] text-white shadow-[0_18px_42px_rgba(151,14,32,0.22)]">
          <div className="flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
            <div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white text-[#d70717] shadow-[0_12px_24px_rgba(0,0,0,0.12)]"><UsersRound className="h-6 w-6" /></span><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-yellow-100">Manajemen Tim</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Akun Marketing</h1><p className="mt-2 max-w-2xl text-xs font-semibold leading-5 text-white/82 sm:text-sm">Pantau pembagian agent, aktivitas terbaru, dan status akun marketing dari satu tempat.</p></div></div>
            <button type="button" onClick={() => setCreateOpen(true)} className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-2xl bg-white px-5 text-sm font-black text-[#d70717] shadow-[0_12px_24px_rgba(0,0,0,0.12)] transition hover:-translate-y-0.5 hover:bg-yellow-50"><UserPlus className="h-5 w-5" /> Tambah Marketing</button>
          </div>
        </header>
        ...
        <section className="mt-4 overflow-hidden rounded-[28px] border border-red-950/[0.06] bg-white shadow-[0_18px_42px_rgba(151,14,32,0.09)]">
          <div className="border-b border-red-950/[0.06] p-4 sm:p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-lg font-black">Daftar Marketing</h2><p className="mt-1 text-xs font-semibold text-slate-500">{visibleAccounts.length} akun ditampilkan</p></div><div className="flex min-w-0 flex-col gap-2 sm:flex-row"><label className="flex h-10 min-w-0 flex-1 items-center gap-2 rounded-2xl border border-red-950/[0.06] bg-[#fffafa] px-3 sm:w-72"><Search className="h-4 w-4 shrink-0 text-[#d70717]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari nama, email, atau nomor" className="min-w-0 flex-1 bg-transparent text-xs font-semibold outline-none placeholder:text-slate-400" /></label><select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} className="h-10 rounded-2xl border border-red-950/[0.06] bg-white px-3 text-xs font-bold text-slate-700 outline-none"><option value="all">Semua status</option><option value="active">Aktif</option><option value="inactive">Nonaktif</option></select><button type="button" onClick={() => void loadAccounts()} disabled={loading} title="Muat ulang" className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-red-100 bg-red-50 text-[#d70717] disabled:opacity-60"><RefreshCw className={loading ? "h-4 w-4 animate-spin" : "h-4 w-4"} /></button></div></div></div>
          {error ? <div className="m-4 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs font-bold text-rose-700 sm:m-5">{error}</div> : null}
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-[#fff1ed] text-[10px] font-black uppercase tracking-[0.12em] text-red-900"><tr><th className="px-5 py-3">Marketing</th><th className="px-4 py-3">Kontak</th><th className="px-4 py-3">Agent Binaan</th><th className="px-4 py-3">Aktivitas Terakhir</th><th className="px-4 py-3">Status</th><th className="px-5 py-3 text-right">Aksi</th></tr></thead><tbody>
            {visibleAccounts.map((account) => <tr key={account.id} className="border-t border-red-950/[0.05] hover:bg-red-50/40"><td className="px-5 py-4"><AccountIdentity account={account} /></td><td className="px-4 py-4"><p className="inline-flex items-center gap-2 font-semibold text-slate-700"><Mail className="h-3.5 w-3.5 text-[#d70717]" />{account.email || "-"}</p><p className="mt-1 inline-flex items-center gap-2 text-[10px] font-semibold text-slate-500"><Phone className="h-3 w-3" />{account.phone || "Belum tersedia"}</p></td><td className="px-4 py-4"><p className="font-black text-slate-900">{account.agent_count || 0} agent</p><p className="mt-1 text-[10px] font-semibold text-[#d70717]">{account.active_agent_count || 0} aktif</p></td><td className="px-4 py-4 font-semibold text-slate-600">{formatDateTime(account.last_agent_transaction_at)}</td><td className="px-4 py-4"><StatusBadge active={account.aktif} /></td><td className="px-5 py-4 text-right"><button onClick={() => setSelected(account)} className="inline-flex h-9 items-center gap-2 rounded-2xl border border-red-100 bg-white px-3 font-black text-[#d70717] shadow-sm"><Pencil className="h-3.5 w-3.5" /> Kelola</button></td></tr>)}
            <EmptyRows loading={loading} empty={visibleAccounts.length === 0} colSpan={6} />
          </tbody></table></div>
          <div className="divide-y divide-red-950/[0.05] md:hidden">{visibleAccounts.map((account) => <article key={account.id} className="p-4"><div className="flex items-start justify-between gap-3"><AccountIdentity account={account} /><StatusBadge active={account.aktif} /></div><div className="mt-4 grid grid-cols-2 gap-2"><Info label="Agent binaan" value={`${account.agent_count || 0} agent`} /><Info label="Aktivitas terakhir" value={formatDateTime(account.last_agent_transaction_at)} /></div><button onClick={() => setSelected(account)} className="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#d70717,#ff6a00)] text-xs font-black text-white"><Pencil className="h-4 w-4" /> Kelola Akun</button></article>)}{!loading && visibleAccounts.length === 0 ? <p className="px-4 py-14 text-center text-xs font-semibold text-slate-400">Akun marketing tidak ditemukan.</p> : null}{loading ? <p className="px-4 py-14 text-center text-xs font-semibold text-slate-400">Memuat akun marketing...</p> : null}</div>
        </section>
      </div>
      <RegisterMemberModal open={createOpen} onClose={() => setCreateOpen(false)} fixedRole="marketing" title="Tambah Marketing" subtitle="Buat akun marketing NuansaPulsa untuk memantau agent binaan." theme="retail" createEndpoint="/api/operator/marketing/create" onSuccess={loadAccounts} />
...
function ManageMarketingModal({ account, onClose, onSaved }: { account: MarketingAccount | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <AppModal open={Boolean(account)} onClose={onClose} title="Kelola Akun Marketing" subtitle={account ? `${account.nama || "Marketing"} · ${account.email}` : ""} theme="retail" maxWidthClassName="max-w-3xl" footer={<div className="flex w-full flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button onClick={onClose} className="h-11 rounded-2xl border border-slate-200 px-5 text-sm font-black">Tutup</button><button onClick={() => void saveAccount()} disabled={saving} className="h-11 rounded-2xl bg-[linear-gradient(135deg,#d70717,#ff6a00)] px-5 text-sm font-black text-white disabled:opacity-60">{saving ? "Menyimpan..." : "Simpan Perubahan"}</button></div>}>
...
function Field({ label, value, onChange, type = "text", placeholder = "" }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string }) { return <label className="grid gap-2"><span className="text-xs font-black text-slate-700">{label}</span><input value={value} onChange={(event) => onChange(event.target.value)} type={type} placeholder={placeholder} className="h-11 rounded-2xl border border-red-950/[0.06] px-3 text-sm font-bold outline-none focus:border-[#d70717] focus:ring-4 focus:ring-red-100" /></label>; }
function AccountIdentity({ account }: { account: MarketingAccount }) { return <div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-[#fff1ed] font-black text-[#d70717] ring-1 ring-red-100">{(account.nama || "M").slice(0, 1).toUpperCase()}</span><div className="min-w-0"><p className="truncate font-black">{account.nama || "Marketing"}</p><p className="mt-0.5 text-[10px] font-semibold text-slate-400">ID #{account.id} · {formatDate(account.dibuat_pada)}</p></div></div>; }
function StatusBadge({ active }: { active: boolean }) { return <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-black ${active ? "bg-red-100 text-red-800" : "bg-rose-100 text-rose-700"}`}><CircleCheck className="h-3.5 w-3.5" />{active ? "Aktif" : "Nonaktif"}</span>; }
function Summary({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: number; tone: "red" | "blue" | "amber" | "orange" }) { const tones = { red: "bg-red-100 text-[#d70717]", blue: "bg-sky-100 text-sky-700", amber: "bg-amber-100 text-amber-700", orange: "bg-orange-100 text-orange-700" }; return <div className="flex min-h-24 items-center justify-between rounded-[24px] border border-red-950/[0.06] bg-white p-4 shadow-[0_12px_28px_rgba(151,14,32,0.07)]"><div><p className="text-[9px] font-black uppercase tracking-[0.08em] text-slate-500 sm:text-[10px]">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${tones[tone]}`}>{icon}</span></div>; }
function Info({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl bg-[#fffafa] p-3 ring-1 ring-red-950/[0.05]"><p className="text-[9px] font-black uppercase text-slate-400">{label}</p><p className="mt-1 text-xs font-black text-slate-800">{value}</p></div>; }
