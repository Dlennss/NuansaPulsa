...
function statToneClass(tone: StatTone) {
  if (tone === "orange") return "border-l-orange-500 border-red-100 bg-white text-slate-950";
  if (tone === "rose") return "border-l-rose-500 border-red-100 bg-white text-slate-950";
  if (tone === "amber") return "border-l-amber-400 border-red-100 bg-white text-slate-950";
  return "border-l-[#e70717] border-red-100 bg-white text-slate-950";
}

function iconToneClass(tone: StatTone) {
  if (tone === "orange") return "bg-orange-50 text-[#ff6a00] ring-2 ring-orange-100";
  if (tone === "rose") return "bg-rose-50 text-[#e70717] ring-2 ring-rose-100";
  if (tone === "amber") return "bg-amber-50 text-[#d97706] ring-2 ring-amber-100";
  return "bg-red-50 text-[#e70717] ring-2 ring-red-100";
}
...
export default function AdminHome() {
  ...
  const stats = [
    { title: "Akun Pengguna", value: fmtNumber(data.retailCount), icon: Users, tone: "red" as StatTone, desc: "User dan agent NuansaPulsa" },
  ...
  return (
    <div className="-m-2 min-h-screen bg-[#fff6f4] p-3 text-slate-950 sm:p-5 lg:p-6">
      <div className="mx-auto max-w-7xl space-y-5">
        <section className="overflow-hidden rounded-[30px] border border-red-100 bg-white shadow-[0_24px_60px_rgba(151,14,32,0.12)]">
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_330px]">
            <div className="relative overflow-hidden bg-[linear-gradient(135deg,#c90016_0%,#ef1717_54%,#ff7a00_130%)] p-5 text-white sm:p-7">
              <div className="pointer-events-none absolute -right-12 -top-16 h-56 w-56 rounded-full border border-white/20 bg-white/10" />
              <div className="pointer-events-none absolute bottom-0 right-16 h-28 w-28 rounded-full border border-yellow-200/30" />
              ...
              <p className="relative mt-3 max-w-2xl text-sm font-semibold leading-6 text-white/90">
                Pantau pengguna, kredit agent, transaksi, saldo, dan produk dari satu dashboard operasional NuansaPulsa.
              </p>
              <div className="relative mt-5 flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-2 rounded-full border border-white bg-white px-3 py-1.5 text-xs font-black text-[#d70717]">
                  <BadgeCheck className="h-4 w-4" />
                  Super Admin aktif
                </span>
                <span className="inline-flex items-center gap-2 rounded-full border border-yellow-200/70 bg-yellow-300 px-3 py-1.5 text-xs font-black text-[#9f0714]">
            ...
            <div className="flex flex-col justify-center gap-3 bg-[#fff7f5] p-5 sm:p-7">
              <div className="rounded-[24px] border border-red-100 bg-white p-4 shadow-[0_14px_30px_rgba(151,14,32,0.08)]">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#d70717]">Kesehatan Sistem</p>
                <p className="mt-2 text-2xl font-black text-slate-950">{loading ? "Memuat..." : "Tersinkron"}</p>
                <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">Data dashboard diambil langsung dari API internal NuansaPulsa.</p>
              </div>
              <Link
                href="/dashboard/admin/integrasi/pulsa24jam"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border-2 border-[#d70717] bg-white px-4 py-3 text-sm font-black text-[#d70717] shadow-[0_12px_24px_rgba(151,14,32,0.10)] outline-none transition hover:bg-red-50 focus-visible:ring-4 focus-visible:ring-red-200"
              >
                Koneksi Pulsa24Jam
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((item) => {
            ...
            return (
              <div key={item.title} className={`rounded-[24px] border border-l-4 p-4 shadow-[0_16px_34px_rgba(151,14,32,0.07)] ${statToneClass(item.tone)}`}>
          ...
          {[
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="group rounded-[24px] border border-red-100 bg-white p-5 shadow-[0_16px_34px_rgba(151,14,32,0.07)] outline-none transition hover:-translate-y-0.5 hover:border-red-300 hover:shadow-[0_20px_40px_rgba(151,14,32,0.11)] focus-visible:ring-4 focus-visible:ring-red-200"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-red-50 text-[#d70717] ring-2 ring-red-100 transition group-hover:bg-[#d70717] group-hover:text-white">
                    <Icon className="h-5 w-5" />
                  </span>
                </div>
              </Link>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          <Link
            ...
            className="group flex min-h-28 items-center justify-between gap-4 rounded-[24px] border border-red-100 bg-white p-5 shadow-[0_16px_34px_rgba(151,14,32,0.07)] transition hover:border-red-300"
          >
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#d70717]">Pemantauan Lapangan</p>
            ...
            <ArrowUpRight className="h-6 w-6 shrink-0 text-[#d70717]" />
          </Link>
          <Link
            href="/dashboard/admin/pemantauan-tim?role=operator_credit"
            className="group flex min-h-28 items-center justify-between gap-4 rounded-[24px] border border-orange-100 bg-white p-5 shadow-[0_16px_34px_rgba(151,14,32,0.07)] transition hover:border-orange-300"
          >
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#ff6a00]">Pengawasan Kredit</p>
            ...
            <ArrowUpRight className="h-6 w-6 shrink-0 text-[#ff6a00]" />
          </Link>
        </section>
      </div>
    </div>
