  ...
  return (
    <main className="-m-2 min-h-screen bg-[#eef7f2] p-3 text-slate-950 sm:p-5 lg:p-7">
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <header className="overflow-hidden rounded-[28px] bg-[linear-gradient(135deg,#053a2f_0%,#087a50_58%,#50cf3e_100%)] px-5 py-7 text-white shadow-[0_22px_50px_rgba(151,14,32,0.16)] sm:px-8">
          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-amber-100">Pengawasan Admin</p>
          <div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              <h1 className="text-3xl font-black tracking-normal sm:text-4xl">Pemantauan Operasional Kredit</h1>
              <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-red-50/90">
        ...
        <section>
          <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
            <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
              ...
              <Link href="/dashboard/admin/master/members" className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-red-400 bg-red-50 px-3 text-xs font-black !text-red-900"><UserCog className="h-4 w-4" /> Kelola Akun</Link>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[780px] text-left text-sm">
                <thead className="bg-[#e5f6ee] text-[11px] font-black uppercase text-red-900"><tr><th className="px-5 py-3">Marketing</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-center">Ditangani</th><th className="px-4 py-3 text-center">Ke Operator</th><th className="px-4 py-3 text-center">Disetujui</th><th className="px-5 py-3">Aktivitas Terakhir</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {marketing.map((member) => {
                    ...
                    return <tr key={member.id} className="hover:bg-red-50/40"><td className="px-5 py-4"><p className="font-black">{member.nama || "Marketing"}</p><p className="mt-1 text-xs text-slate-600">{member.email || member.phone || "-"}</p></td><td className="px-4 py-4"><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-black ${member.aktif ? "bg-red-100 text-red-800" : "bg-rose-100 text-rose-800"}`}>{member.aktif ? <CheckCircle2 className="h-3.5 w-3.5" /> : <XCircle className="h-3.5 w-3.5" />}{member.aktif ? "Aktif" : "Nonaktif"}</span></td><td className="px-4 py-4 text-center font-black">{assigned.length}</td><td className="px-4 py-4 text-center font-black">{forwarded.length}</td><td className="px-4 py-4 text-center font-black text-red-700">{approved.length}</td><td className="px-5 py-4 text-xs font-semibold text-slate-600">{dateTime(last?.created_at)}</td></tr>;
                  })}
        ...
        <section>
          <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
            <div className="border-b border-slate-200 px-5 py-4"><h2 className="flex items-center gap-2 text-lg font-black"><Activity className="h-5 w-5 text-red-700" /> Aktivitas Kredit Terbaru</h2></div>
            <div className="max-h-[360px] divide-y divide-slate-100 overflow-y-auto">
              {activities.slice(0, 12).map((item) => <article key={item.id} className="grid gap-2 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_auto]"><div><div className="flex flex-wrap items-center gap-2"><p className="font-black">{item.actor_name || item.actor_email || roleLabel(item.actor_role)}</p><span className="rounded-full bg-red-100 px-2 py-1 text-[11px] font-black text-red-800">{roleLabel(item.actor_role)}</span></div><p className="mt-1 text-sm font-bold text-slate-700">{actionLabel(item.action)}</p>{item.reason ? <p className="mt-1 text-xs font-semibold text-slate-600">Catatan: {item.reason}</p> : null}</div><div className="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><Clock3 className="h-3.5 w-3.5" /> {dateTime(item.created_at)}</div></article>)}
