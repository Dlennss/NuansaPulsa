...
export default function AdminBankPage() {
  ...
  if (!isOperatorMode) {
    bankColumns.push({
      ...
      render: (item) => (
        <span className="rounded-md border border-red-300 bg-red-100 px-2 py-1 font-bold text-red-900">
    ...
    bankColumns.push({
      ...
      render: (item) =>
        item.aktif ? (
          <span className="rounded-full border border-red-400 bg-red-100 px-2 py-0.5 text-xs font-bold text-red-900">
            Aktif
          </span>
        ) : (
  ...
  const historyColumns: DataTableColumn<HistoryRow>[] = [
    ...
    { id: "ref_id", header: "RefID", tdClassName: "whitespace-nowrap font-mono text-xs text-slate-300", render: (item) => item.ref_id },
    {
      ...
      tdClassName: "whitespace-nowrap",
      render: (item) => (
        <span className={item.arah.toLowerCase() === "credit" ? "font-semibold text-red-800" : "font-semibold text-rose-800"}>
          {item.arah}
        </span>
      ),
    },
  ...
  return (
    <div className="space-y-4 p-2">
      <div className="grid gap-3 md:grid-cols-[1.4fr_1fr]">
        <div className="rounded-2xl border border-red-900 bg-[#b20717] p-5 shadow-[0_16px_40px_-24px_rgba(151,14,32,0.35)]">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="grid h-12 w-12 place-items-center rounded-2xl border border-white/25 bg-white/10 text-amber-300">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-white/25 bg-white/10 px-4 py-4">
              <div className="text-sm font-semibold text-red-100">{isOperatorMode ? "Data Rekening" : "Total Saldo Bank"}</div>
              <div className="mt-1 text-2xl font-black text-white">{isOperatorMode ? "Terbatas" : `Rp ${fmtID(totalSaldoBank)}`}</div>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-300 px-4 py-4">
              <div className="text-sm font-semibold text-red-950">Jumlah Bank</div>
              <div className="mt-1 text-2xl font-black text-red-950">{items.length}</div>
            </div>
          </div>
        </div>

        <div className="rounded-2xl border border-red-200 bg-white p-5 shadow-[0_16px_40px_-24px_rgba(151,14,32,0.18)]">
          <div className="text-base font-bold text-red-950">{isWalletMode ? "Akses Operator Wallet" : isOperatorMode ? "Akses Operator Transaksi" : "Aksi Cepat"}</div>
          <div className="mt-1 text-sm font-medium text-red-900/75">
            {isWalletMode
      ...
      <div className="rounded-2xl border border-white/10 bg-card/80 shadow-[0_16px_40px_-24px_rgba(0,0,0,0.8)]">
        <div className="border-b border-white/10 bg-linear-to-r from-white/10 via-white/5 to-transparent px-4 py-3">
          ...
          <div className="text-xs font-medium text-red-900/70">
            {isWalletMode
                ...
                {isWalletMode ? (
                  <>
                    ...
                    {canTransferToBCAOperational(actionMenu.item) ? (
                      <button
                        type="button"
                        className="mt-1 flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm font-semibold text-red-800 transition hover:bg-red-50"
                        onClick={() => {
                          const item = actionMenu.item;
                          setActionMenu(null);
      ...
      >
        <div className="grid gap-3">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-sm">
            ...
            <div className="font-medium text-red-950">{manualBank?.atas_nama || "-"}</div>
            <div className="font-mono text-red-700">{manualBank?.nomor_rekening || "-"}</div>
            <div className="font-semibold text-red-700">Saldo: Rp {fmtID(Number(manualBank?.saldo || 0))}</div>
          </div>
      ...
      >
        <div className="grid gap-3">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-sm">
            ...
            <div className="font-medium text-red-950">{providerTopupBank?.atas_nama || "-"}</div>
            <div className="font-mono text-red-700">{providerTopupBank?.nomor_rekening || "-"}</div>
            <div className="font-semibold text-red-700">Saldo rekening: Rp {fmtID(Number(providerTopupBank?.saldo || 0))}</div>
          </div>
          <div className="grid gap-1.5">
        ...
        <div className="grid gap-3">
          {isWalletMode ? (
            ...
            <>
              ...
              <div className="grid gap-1.5">
                <label className="text-sm font-medium">Nama Rekening</label>
                <Input value={atasNama} onChange={(e) => setAtasNama(e.target.value)} placeholder="Contoh: PT NuansaPulsa" />
              </div>
              <div className="grid gap-1.5">
                <label className="text-sm font-medium">Nomor Rekening</label>
