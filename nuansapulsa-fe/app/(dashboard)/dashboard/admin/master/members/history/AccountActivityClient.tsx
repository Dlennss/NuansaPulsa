  ...
  return (
    <main className="-m-2 min-h-screen bg-[#eef7f2] p-3 text-slate-950 sm:p-5 lg:p-7">
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <header className="overflow-hidden rounded-[28px] border border-red-100 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.07)]">
          <div className="bg-[linear-gradient(135deg,#053a2f_0%,#087a50_62%,#50cf3e_100%)] px-5 py-6 text-white sm:px-7">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div><p className="text-[11px] font-black uppercase tracking-[0.18em] text-amber-100">Pengawasan Akun</p><h1 className="mt-2 text-3xl font-black">Aktivitas Akun</h1><p className="mt-2 text-sm font-semibold text-red-50/90">Riwayat transaksi dan pergerakan saldo pengguna NuansaPulsa.</p></div>
              <div className="flex flex-wrap gap-2"><button type="button" onClick={() => router.push("/dashboard/admin/master/members")} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-3 text-sm font-black text-white hover:bg-white/20"><ArrowLeft className="h-4 w-4" /> Kembali</button><button type="button" onClick={() => void load()} disabled={loading} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-white px-3 text-sm font-black text-red-800 hover:bg-red-50"><RefreshCcw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Muat Ulang</button></div>
            </div>
          </div>
          <div className="grid gap-4 p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-6">
            <div className="flex min-w-0 items-center gap-3"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-red-50 text-red-700"><UserCog className="h-6 w-6" /></span><div className="min-w-0"><p className="truncate text-lg font-black">{member?.nama || (loading ? "Memuat akun..." : "Akun tidak ditemukan")}</p><p className="truncate text-sm font-semibold text-slate-500">{member?.email || "-"}</p></div></div>
            {member ? <div className="flex flex-wrap gap-2 sm:justify-end"><span className="rounded-full bg-red-50 px-3 py-1.5 text-xs font-black text-red-700">{roleLabel(member.role)}</span><span className={`rounded-full px-3 py-1.5 text-xs font-black ${member.aktif ? "bg-amber-50 text-amber-700" : "bg-rose-50 text-rose-700"}`}>{member.aktif ? "Aktif" : "Nonaktif"}</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600">Bergabung {shortDate(member.dibuat_pada)}</span></div> : null}
          </div>
        </header>
        ...
        <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
          <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between">
            <nav className="flex flex-wrap gap-2" aria-label="Jenis aktivitas">{([{ value: "overview", label: "Ringkasan" }, { value: "mutasi", label: "Saldo & Mutasi" }, { value: "transaksi", label: "Transaksi" }] as Array<{ value: Tab; label: string }>).map((item) => <button key={item.value} type="button" onClick={() => setTab(item.value)} className={`min-h-10 rounded-full border px-4 text-sm font-black transition ${tab === item.value ? "border-red-700 bg-red-700 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-red-300"}`}>{item.label}</button>)}{roleAction ? <Link href={roleAction.href} className="inline-flex min-h-10 items-center gap-1.5 rounded-full border border-amber-300 bg-amber-50 px-4 text-sm font-black text-amber-800"><ShieldCheck className="h-4 w-4" /> {roleAction.label}</Link> : null}</nav>
            {tab !== "overview" ? <div className="relative"><button type="button" onClick={() => setDownloadOpen((value) => !value)} disabled={exporting} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 text-sm font-black text-red-800"><Download className="h-4 w-4" /> {exporting ? "Menyiapkan..." : "Unduh Laporan"}</button>{downloadOpen ? <div className="absolute right-0 top-12 z-20 w-40 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">{(["csv", "excel", "pdf"] as const).map((kind) => <button key={kind} type="button" onClick={() => void exportCurrent(kind)} className="block w-full rounded-lg px-3 py-2 text-left text-sm font-bold uppercase text-slate-700 hover:bg-red-50">{kind}</button>)}</div> : null}</div> : null}
          </div>

          {tab !== "overview" ? <form onSubmit={applyFilter} className="grid gap-3 border-b border-slate-200 bg-[#fff7f5] p-4 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1fr)_160px_170px_170px_auto]">
            <label className="relative"><span className="sr-only">Cari aktivitas</span><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari referensi, produk, atau tujuan" className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-sm outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100" /></label>
            {tab === "mutasi" ? <select value={direction} onChange={(event) => setDirection(event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-red-500"><option value="">Semua arah</option><option value="credit">Dana masuk</option><option value="debit">Dana keluar</option></select> : <div className="hidden lg:block" />}
            <input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-red-500" aria-label="Tanggal mulai" />
            <input type="date" value={to} onChange={(event) => setTo(event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-red-500" aria-label="Tanggal akhir" />
            <div className="flex gap-2"><button type="submit" className="min-h-11 flex-1 rounded-xl bg-red-700 px-4 text-sm font-black text-white hover:bg-red-800">Terapkan</button><button type="button" onClick={resetFilter} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-black text-slate-600 hover:bg-slate-50">Reset</button></div>
          </form> : null}

          <div className="overflow-x-auto">
            {loading ? <div className="grid min-h-64 place-items-center text-sm font-bold text-slate-500"><Loader2 className="mb-2 h-6 w-6 animate-spin text-red-600" /> Memuat aktivitas...</div> : tab === "overview" ? <ActivityTimeline rows={recentActivity} /> : tab === "mutasi" ? <MutationTable rows={mutations} /> : <TransactionTable rows={transactions} />}
          </div>
...
function ActivityTimeline({ rows }: { rows: ActivityItem[] }) {
  ...
  return <div className="divide-y divide-slate-100">{rows.map((item) => <article key={item.id} className="grid gap-3 px-4 py-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:px-5"><span className={`grid h-10 w-10 place-items-center rounded-xl ${item.kind === "mutasi" ? "bg-red-50 text-red-700" : "bg-sky-50 text-sky-700"}`}>{item.kind === "mutasi" ? <WalletCards className="h-5 w-5" /> : <History className="h-5 w-5" />}</span><div className="min-w-0"><p className="font-black text-slate-900">{item.title}</p><p className="mt-1 truncate text-xs font-semibold text-slate-500">{item.detail}</p><p className="mt-1 flex items-center gap-1 text-xs text-slate-400"><Clock3 className="h-3.5 w-3.5" /> {dateTime(item.time)}</p></div><div className="sm:text-right"><p className="font-black text-slate-900">{money(item.amount)}</p><Status value={item.status} /></div></article>)}</div>;
}

function MutationTable({ rows }: { rows: MutasiRow[] }) {
  if (!rows.length) return <EmptyState />;
  return <table className="w-full min-w-[850px] text-left text-sm"><thead className="bg-red-50 text-[11px] font-black uppercase text-red-800"><tr><th className="px-4 py-3">Waktu</th><th className="px-4 py-3">Aktivitas</th><th className="px-4 py-3">Referensi</th><th className="px-4 py-3">Nominal</th><th className="px-4 py-3">Arah</th><th className="px-4 py-3">Saldo</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((item) => <tr key={item.id} className="hover:bg-red-50/40"><td className="whitespace-nowrap px-4 py-4 text-slate-600">{dateTime(item.dibuat_pada)}</td><td className="px-4 py-4"><p className="font-bold">{item.alasan || "Mutasi saldo"}</p><p className="mt-1 text-xs text-slate-500">{item.catatan || "-"}</p></td><td className="whitespace-nowrap px-4 py-4 font-mono text-xs text-slate-500">{item.ref_id || "-"}</td><td className="whitespace-nowrap px-4 py-4 font-black">{money(item.jumlah)}</td><td className="px-4 py-4"><Status value={item.arah} /></td><td className="whitespace-nowrap px-4 py-4 text-slate-600">{money(Number(item.saldo_sebelum || 0))} → {money(Number(item.saldo_sesudah || 0))}</td></tr>)}</tbody></table>;
}

function TransactionTable({ rows }: { rows: TrxRow[] }) {
  if (!rows.length) return <EmptyState />;
  return <table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-red-50 text-[11px] font-black uppercase text-red-800"><tr><th className="px-4 py-3">Waktu</th><th className="px-4 py-3">Produk</th><th className="px-4 py-3">Referensi</th><th className="px-4 py-3">Tujuan</th><th className="px-4 py-3">Nominal</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((item) => <tr key={item.id} className="hover:bg-red-50/40"><td className="whitespace-nowrap px-4 py-4 text-slate-600">{dateTime(item.dibuat_pada)}</td><td className="px-4 py-4"><p className="font-bold">{item.kode_produk || item.perintah || "Transaksi"}</p><p className="mt-1 text-xs text-slate-500">{item.keterangan || "-"}</p></td><td className="whitespace-nowrap px-4 py-4 font-mono text-xs text-slate-500">{item.ref_id || "-"}</td><td className="whitespace-nowrap px-4 py-4">{item.tujuan || "-"}</td><td className="whitespace-nowrap px-4 py-4 font-black">{money(Number(item.biaya_aktual || item.biaya_perkiraan || 0))}</td><td className="px-4 py-4"><Status value={item.status} /></td></tr>)}</tbody></table>;
}
...
function Status({ value }: { value: string }) {
  ...
  return <span className={`mt-1 inline-flex rounded-full px-2.5 py-1 text-[11px] font-black ${positive ? "bg-red-50 text-red-700" : negative ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>{normalized === "credit" ? "Dana masuk" : normalized === "debit" ? "Dana keluar" : value || "Diproses"}</span>;
}

function EmptyState() {
  return <div className="grid min-h-64 place-items-center px-5 py-12 text-center"><div><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-red-50 text-red-700"><History className="h-6 w-6" /></span><p className="mt-3 font-black text-slate-800">Belum ada aktivitas</p><p className="mt-1 text-sm font-semibold text-slate-500">Aktivitas akun akan muncul setelah transaksi atau perubahan saldo terjadi.</p></div></div>;
}
